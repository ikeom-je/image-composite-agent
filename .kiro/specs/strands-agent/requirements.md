...
Image Compositorの全機能を自然言語で操作できるAIチャットエージェントを、Strands Agents SDKとAnthropic Claude APIを用いて構築する。

---

## Req 1: エージェントコア基盤

**説明**: Strands Agents SDKを使用したAIエージェントのコアシステム。Anthropic Claude APIを直接利用し、自然言語で画像合成システムの操作を行う。

**Acceptance Criteria**:
- AC 1.1: Strands Agents SDK (`strands-agents`) を使用してエージェントが初期化できること
- AC 1.2: Anthropic Claude (Sonnet) をモデルプロバイダーとして設定できること
- AC 1.3: 日本語のシステムプロンプトでエージェントが応答すること
- AC 1.4: `@tool` デコレータで定義されたカスタムツールをエージェントが呼び出せること
- AC 1.5: Lambda関数としてデプロイ可能であること（Python 3.11）
...
**Acceptance Criteria**:
- AC 5.1: 会話メッセージ（user/assistant/tool_use/tool_result）をDynamoDBに保存できること
- AC 5.2: セッションIDで会話履歴を取得できること
- AC 5.3: セッションIDで会話履歴を削除できること
...
**説明**: Anthropic APIキーをAWS Secrets Managerで安全に管理する。

**Acceptance Criteria**:
- AC 7.1: APIキーがSecrets Managerに保存されていること
- AC 7.2: Agent LambdaがIAMロールでSecrets Managerから取得できること
- AC 7.3: APIキーがソースコードや環境変数にハードコードされていないこと
- AC 7.4: CDKでSecrets ManagerリソースとIAMポリシーが定義されていること
...
**Acceptance Criteria**:
- AC 8.1: Agent Lambda関数がCDKで定義されていること（Python 3.11, 512MB, 90秒, ARM64）
- AC 8.2: DynamoDBテーブルがCDKで定義されていること（PAY_PER_REQUEST, TTL有効）
- AC 8.3: API Gatewayに `/chat` リソースが追加されていること
- AC 8.4: Secrets Managerシークレットがパラメータで参照可能であること
- AC 8.5: Lambda LayerでStrands Agent依存パッケージが含まれていること
...
**Acceptance Criteria**:
- AC 11.1: Anthropic API呼び出し失敗時にユーザーフレンドリーなメッセージを返すこと
...
- AC 12.4: Anthropic APIのモック/スタブでユニットテストが実行可能であること
