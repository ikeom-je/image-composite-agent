...
- AC 2.3: ベース画像（test/transparent/white/#RRGGBB/#RRGGBBAA/S3 URL/HTTP URL）を指定できること
...
- AC 2.7: テキストオーバーレイ（text1〜text3）を指定して合成画像にテロップを追加できること
- AC 2.8: テキストの位置を自然言語（「左下」等）または座標で指定できること
- AC 2.9: ベース画像の透明度（base_opacity: 0-100）を指定できること
...
- AC 3.5: テキストオーバーレイ付きの動画を生成できること
...
**Acceptance Criteria**:
- AC 6.1: `POST /chat` でエージェントにメッセージを送信できること（リクエスト形式: `{ "message": string, "sessionId"?: UUIDv4, "modelId"?: string }`、メッセージは2000文字以内）
...
- AC 6.6: `GET /chat/models` で利用可能モデル一覧を取得できること（Req 13参照）
...
- AC 7.3: `aws-marketplace:ViewSubscriptions`、`aws-marketplace:Subscribe`、`aws-marketplace:Unsubscribe` 権限が設定されていること（Bedrockモデル自動サブスクリプション用）
...
## Req 13: 動的モデル選択（マルチモデル対応）

**説明**: フロントエンドから利用可能モデル一覧を取得し、リクエストごとにモデルを切り替えられるようにする。許可リスト方式でモデルIDを管理し、インジェクションを防止する。

**Acceptance Criteria**:
- AC 13.1: `GET /chat/models` でJSON形式のモデル一覧を返却すること
...
- AC 13.2: モデルIDは許可リスト（`ALLOWED_MODELS`）方式で管理すること（インジェクション防止）
- AC 13.3: `POST /chat` のリクエストボディに `modelId` を指定でき、許可リスト外の値は400エラー（`Invalid modelId`）を返すこと
- AC 13.4: `modelId` 未指定時は環境変数 `AGENT_MODEL_ID`（許可リスト外なら内部フォールバック値）を使用すること
