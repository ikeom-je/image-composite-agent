...
API側 (`image_processor.py` / `image_compositor.py`) で画像が省略されているかをもとにモードを決定する:

```python
def determine_image_mode(query_params, has_text):
    ...
    if has_image3:
        ...
        return 'text_only_or_image1_with_text'
    else:
        return 'single'  # image1 のみ + テキストなし → アイコン配置
```
...
| text_placeholders | placeholder なし |
| video.format | MP4 |
| video.duration | 3 |

> フォールバック時の `baseImage` は既存挙動（透明）を維持。JSON が正常に読み込めた場合のみ新仕様（黒背景）が適用される。
